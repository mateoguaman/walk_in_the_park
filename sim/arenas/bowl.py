...
class Bowl(composer.Arena):
    ...
    def _build(self, size=(10, 10), aesthetic='default', name='bowl', slope=0.1):
    ...
    def initialize_episode(self, physics, random_state):
        if self._regenerate:
            ...
            # Generate smooth bowl-shaped terrain
            x = np.linspace(-self._size[0], self._size[0], res)
            y = np.linspace(-self._size[1], self._size[1], res)
            xx, yy = np.meshgrid(x, y)
            
            # Calculate distance from center and apply slope
            distance = np.sqrt(xx**2 + yy**2)
            terrain = self._slope * distance
